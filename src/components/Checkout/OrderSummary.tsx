...
export default function OrderSummary() {
  ...
  const { getCartTotal, getCartCount, cartItems, clearCart } = useCart();
  const {
    cardRef,
    contactInfo,
    note,
  ...
  const [tipMode, setTipMode] = useState<
    "preset" | "manual-percent" | "manual-amount"
  >("preset");
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      if (tokenResult.status !== "OK") {
        const errorMessage =
          tokenResult.errors
            ...
            .join(", ") || "Card verification failed. Please check your details.";
      ...
      if (!response.ok || !result.success) {
        const errorMsg =
          result.details
            ?.map((d: { detail?: string; message?: string }) => d.detail || d.message)
            .join(", ") ||
          result.error ||
      ...
      sessionStorage.setItem(
        ...
        JSON.stringify({
          ...
          contactInfo,
          cartItems,
          amounts: { subtotal, bagFee, tax, tip: calculateTip(), total },
          pickupTime,
          note,
        })
      );
    ...
    <div className="w-full">
      ...
      >
        <h3 className="text-black font-bold text-base">
          Order summary ({getCartCount()} item
          {getCartCount() !== 1 ? "s" : ""})
        </h3>
        <svg
      ...
      <div
        className={`overflow-hidden transition-all duration-300 ease-in-out ${isExpanded ? "max-h-[500px] mb-6 opacity-100" : "max-h-0 opacity-0"}`}
      >
      ...
      <div className="grid grid-cols-4 gap-3 mb-4">
        ...
        <button
          onClick={() => {
            setTipMode(
              tipMode === "preset" ? "manual-percent" : tipMode
            );
      ...
      <div className="border border-gray-400 rounded-tl-lg rounded-br-lg rounded-tr-none rounded-bl-none p-3 flex items-center gap-2 mb-6 bg-white">
        ...
        <input
          ...
          className="flex-1 outline-none text-sm placeholder-black"
        />
      </div>
      ...
      <button
        ...
        className={`w-full font-medium py-3 rounded-tl-lg rounded-br-lg rounded-tr-none rounded-bl-none shadow-sm transition-colors mb-12 ${
          isProcessing || cartItems.length === 0
            ? "bg-gray-300 text-gray-500 cursor-not-allowed"
            : "bg-[#FFCC80] hover:bg-[#ffb74d] text-black"
        }`}
      >
        {isProcessing ? (
